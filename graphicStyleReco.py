...
def ai_loop():
	global is_file_selected, nb_samples, nb_time, total_duration, frequency, selected_file, running, ai_res, s_features, is_mic_on, data
	
	while running:
		if not is_mic_one:
			if is_file_selected and nb_time < total_duration: 
				ai_res, s_features = predict_song_for_graphics(selected_file, "with_contrast", s_features ,nb_time, nb_samples)
				nb_time += 1/frequency
				nb_samples += 1
			else:
				ai_res = []
			...
			ai_res, s_features = predict_song_from_mic(data, "with_contrast", s_features, nb_samples)
			nb_samples += 1
